...
int main() {
  ...
  for (size_t i = 0; i < sizeof(array)/sizeof(int); i++) { //remember sizeof(array)/sizeof(int) works
    std::cout << array[i] << " ";
  }
  std::cout << std::endl;
  ...
  char string1[] {'V', 'a', 'n', 'k', 'a', 't', 'a', ' ', 's', 'u', 'c', 'k', 's'};
  ...
  *dynamicNumber = CONST_SIZE_FOR_TODAY; //variable initialisation
  ...
  for (size_t i = 0; i < CONST_SIZE_FOR_TODAY; i++) { // sizeof(dynamicArray)/sizeof(int) doesn't work, because dynamicArray is int* which has const size
